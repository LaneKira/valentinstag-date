...
export default function Home() {
  ...
  const Widget = ({ title, img, onClick }: { title: string, img?: string, onClick: () => void }) => (
    <div 
      onClick={onClick} 
      className="cursor-pointer group relative h-32 w-full rounded-3xl overflow-hidden shadow-md transition-all active:scale-95 border border-white/50 bg-rose-50"
    >
      {img ? (
  ...
  return (
    <main className="min-h-screen relative overflow-hidden flex items-center justify-center p-4 text-black bg-gradient-to-br from-rose-100 via-white to-pink-100 animate-gradient-slow">
      <div className="max-w-md w-full backdrop-blur-xl bg-white/70 rounded-[2.5rem] shadow-[0_20px_50px_rgba(251,113,133,0.15)] p-8 text-center border border-white/80 relative z-10 overflow-hidden">
        ...
        {step > 1 && (
          <button onClick={goBack} className="absolute top-6 left-6 z-30 bg-white/80 backdrop-blur-md text-rose-500 w-10 h-10 flex items-center justify-center rounded-full shadow-sm border border-rose-100">←</button>
        )}
        ...
        {step === 2 && (
          <div className="animate-in slide-in-from-bottom-8 pt-8 text-center">
            ...
            <div className="bg-white/50 backdrop-blur-md rounded-3xl p-6 mb-8 border border-white inline-block w-full">
              ...
              <div className="relative inline-block bg-white rounded-2xl border-2 border-rose-100 px-4 py-2 shadow-sm">
                <p className="text-[10px] text-rose-400 font-bold uppercase mb-1">Uhrzeit anpassen:</p>
                <input 
                  type="time" 
                  value={time} 
                  onChange={(e) => setTime(e.target.value)} 
                  className="text-4xl font-bold bg-transparent text-rose-600 outline-none cursor-pointer" 
                />
              </div>
            </div>
            <button onClick={() => setStep(3)} className="w-full bg-gray-900 text-white font-bold py-5 rounded-2xl shadow-lg">Weiter ✨</button>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="animate-in fade-in zoom-in pt-8">
            ...
            <h2 className="text-2xl font-bold text-gray-800 mb-6">Ein kleiner Hinweis...</h2>
            <div className="bg-amber-50/80 border border-amber-100 rounded-3xl p-6 text-left text-gray-700 leading-relaxed shadow-sm mb-8">
              <p className="mb-4 font-medium">Liebes Möhrchen,</p>
              <p className="text-sm">
                Bitte achte bei der Wahl deines Dinners und der Aktivität darauf, dass wir uns genügend Zeit nehmen, damit sich alles entspannt ausgeht. 
              </p>
              <p className="text-sm mt-4">
                Du kannst die Uhrzeit oben jederzeit wieder ändern oder anpassen, damit wir einen wirklich **stressfreien Tag** zusammen genießen können! ❤️
              </p>
            </div>
            <button onClick={() => setStep(4)} className="w-full bg-rose-500 text-white font-bold py-5 rounded-2xl shadow-lg">Verstanden & Weiter ✨</button>
          </div>
        )}

        {step === 4 && (
          <div className="animate-in slide-in-from-bottom-8 space-y-4 pt-8">
            ...
            <Widget title="Gemeinsam kochen 👨‍🍳" img={images.foodCooking} onClick={() => {setChoices({...choices, food: 'Gemeinsam kochen'}); setStep(5)}} />
          </div>
        )}

        {step >= 5 && step <= 8 && (
          <div className="animate-in slide-in-from-bottom-8 space-y-3 pt-8 text-left">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">{step === 5 ? "Die Aktivität 🎡" : "Details wählen"}</h2>
            {step === 5 && (
              <>
            ...
            {step === 6 && (
              <>
                <Widget title="Carol" img={images.movieCarol} onClick={() => {setChoices({...choices, detail: 'Carol'}); setStep(9)}} />
                <Widget title="Porträt einer jungen Frau..." img={images.moviePortrait} onClick={() => {setChoices({...choices, detail: 'Porträt einer jungen Frau in Flammen'}); setStep(9)}} />
                <Widget title="Ich bringe einen Film mit 🎬" onClick={() => {setChoices({...choices, detail: 'Eigener Filmvorschlag'}); setStep(9)}} />
              </>
            )}
            {step === 7 && (
              <>
                <Widget title="Schönbrunn" img={images.walkSchoenbrunn} onClick={() => {setChoices({...choices, detail: 'Schönbrunn Spaziergang'}); setStep(9)}} />
                <Widget title="1. Bezirk" img={images.walkDistrict1} onClick={() => {setChoices({...choices, detail: '1. Bezirk Spaziergang'}); setStep(9)}} />
              </>
            )}
            {step === 8 && (
              <>
                <Widget title="Billiard" img={images.gameBilliard} onClick={() => {setChoices({...choices, detail: 'Billiard'}); setStep(9)}} />
                <Widget title="Bowlen" img={images.gameBowling} onClick={() => {setChoices({...choices, detail: 'Bowlen'}); setStep(9)}} />
                <Widget title="Home-Kartenspiele 🃏" img={images.gameCards} onClick={() => {setChoices({...choices, detail: 'Home-Kartenspiele'}); setStep(9)}} />
              </>
            )}
          </div>
        )}
        ...
        {step === 11 && (
          <div className="animate-in zoom-in py-4">
            <h2 className="text-3xl font-black text-gray-900 mb-6 uppercase tracking-tighter">Unser Plan</h2>
